...
const ManageSearchAndRateAppImage = () => {
          ...
          <div className="grid grid-cols-4 gap-4">
            {value.slice(0, 2).map((item: any, index: number) => {
              return (
                <img
                  key={index}
                  className="w-[40px] rounded-lg ml-1"
                  src={item?.url}
                  alt=""
                />
              );
            ...
            {(() => {
              if (record.status === "") {
                return (
                  <div className="flex">
                    <AiFillEye
                      onClick={() => checkGoogleMapRatings(record)}
                      className="cursor-pointer"
                      size={20}
                    />
                    <AiFillDelete
                      onClick={() => deletegooglemapError(record)}
  ...
  const checkGoogleMapRatings = async (data: any) => {
    setIsModalOpen(true);
    const body = {
      urlLocation: data?.link,
      comment: data.comment,
    };
    const result = await checkGoogleMapImageRating(body);
  ...
  return (
    <div className="p-4" id="manage">
      <div className="mb-4">
        <Breadcrumb
          items={[
            { title: "Quản lý nhiệm vụ review google map kèm ảnh" },
            { title: "Nhiệm vụ review google map kèm ảnh" },
          ]}
        ></Breadcrumb>
      </div>
      <div className="mb-4">
      ...
      >
        {checkFollow !== null ? (
          checkFollow?.comment && checkFollow?.rating === "5 stars" ? (
            <div>
              <div className="block text-center text-2xl text-lime-700 font-bold ">
                ...
                <div className="mt-4">
                  <div className="">
                    <div className="text-start">Hình ảnh trong nhiệm vụ</div>
                    <div className="grid grid-cols-4 gap-4 mt-4">
                      {item?.image.map((ite: any) => {
                        return (
                          <img
                            key={ite.url}
                            className="w-36 mr-1"
                  ...
                  <div className="mt-4">
                    <div className="text-start">Hình ảnh trên google map</div>
                    <div className="grid grid-cols-4 gap-4 mt-4">
                      {checkFollow.pictures.map((item: any, index: number) => {
                        return (
                          <img
                            className="w-36 mr-1"
                            src={item}
            ...
            <div>
              <div className="block text-center text-2xl text-red-700 font-bold h-80">
                <div className="block h-16 m-5">
                  {" "}
                  Tài khoản này chưa thực hiện bình luận hoặc chưa đánh giá 5
                  sao
                </div>
                <div className="flex justify-center ">
...
export default ManageSearchAndRateAppImage;
