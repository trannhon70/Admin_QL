...
const removedPart = "https://www.youtube.com/";
...
const ManageYoutube = () => {
  ...
  const handleCheck = async (e: any) => {
    ...
    const result: any = await checkChannelFollowedDone(body);
    setCheckFollow(result.userInfo);
  ...
  return (
    <div className="p-4" id="manage">
      <div className="mb-4">
        <Breadcrumb
          items={[
            { title: "Quản lý nhiệm vụ youtube" },
            { title: "Nhiệm vụ youtube" },
          ]}
        ></Breadcrumb>
      </div>
      <div className="mb-4">
      ...
      >
        {checkFollow !== null ? (
          checkFollow?.completeList.length > 0 ? (
            <div>
