...
const ManageCommentTwitter = () => {
            ...
            {(() => {
              if (record.status === "") {
                return (
                  <div className="flex">
                    <AiFillEye
                      onClick={() => checkTwitterComments(record)}
  ...
  const checkTwitterComments = async (data: any) => {
    setIsModalOpen(true);
    const body = {
      urlVideo: data?.task?.link,
      comment: data.comment,
    };
    const result = await checkTwitterComment(body);
  ...
  return (
    <div className="p-4" id="manage">
      <div className="mb-4">
        <Breadcrumb
          items={[
            { title: "Quản lý nhiệm vụ comment twitter" },
            { title: "Nhiệm vụ comment twitter" },
          ]}
        ></Breadcrumb>
      </div>
      <div className="mb-4">
...
export default ManageCommentTwitter;
