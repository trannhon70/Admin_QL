...
const ComponentMakeProgressGoogleSearch = () => {
  ...
  const onclickMakeProgress = async () => {
    ...
    if (fileList.length === 0) {
      //  setError({ ...error, keyword: "error" });
      api.error({
        message: "Hình ảnh mô tả không được bỏ trống!",
      });
      hasError = true;
    }
  ...
  return (
    <div className="mt-2">
      ...
      <div className="mt-2">
        <div className="text-lg text-green-500 font-bold">
          Từ khóa (nằm ở trang 1-5 để user tìm thấy)
        </div>
        <Input.TextArea
          rows={3}
          className=" mt-1"
...
export default ComponentMakeProgressGoogleSearch;
