...
function TransferTable({ pageIndex, setPageIndex, pageSize, setPageSize, handleSearch }: PropsType) {
   const columns: any = [
      {
         title: "STT",
         dataIndex: "index",
   ...
   const transfers = useSelector((state: any) => state.topUp.transfers) || [];

   const items = transfers?.items?.map((item: any, id: number) => {
      return {
         index: id + 1,
         key: id,
         ...
         bodyTransfer: <p className="line-clamp-1">{item.bodyTransfer}</p>,
         created_at: (
